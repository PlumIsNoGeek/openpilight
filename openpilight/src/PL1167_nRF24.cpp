...
int PL1167_nRF24::recalc_parameters()
{
  ...
  if (packet_length > (int)sizeof(_packet) || nrf_address_length < 3) {
    return -1;
  }
...
int PL1167_nRF24::transmit(uint8_t channel)
{
  if (channel != _channel) {
    ...
    _radio.setChannel(2 + _channel); //is faster, because rest of configuration remains static, and does not need to be resent everytime
  }
...
int PL1167_nRF24::internal_receive()
{
  ...
  open();  //be very carefull, RF24::begin call may build instances of objects and this then creates a memory leak!
