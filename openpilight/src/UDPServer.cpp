// UDP Server -- classes to ease handling sockets
 ...
 *
 * \exception udp_server_runtime_error
 * The udp_server_runtime_error exception is raised when the address
...
{
    return ::recvfrom(f_socket, msg, max_size, 0, &src_addr, &src_addr_len);
}

int UDPServer::respond(const char *msg, size_t size)
{
	return sendto(f_socket, msg, size, 0, (struct sockaddr*)&src_addr, src_addr_len);
}
 ...
 * it is possible to setup the socket as non-blocmax_wait_msking if necessary for
 * some other reason.
 *
...
int UDPServer::timed_recv(char *msg, size_t max_size, int max_wait_ms)
{
    fd_set read;
    FD_ZERO(&read);
    FD_SET(f_socket, &read);
    struct timeval timeout;
    timeout.tv_sec = max_wait_ms / 1000;
    timeout.tv_usec = (max_wait_ms % 1000) * 1000;
    int retval = select(f_socket+1, &read, NULL, NULL, &timeout);
    if(retval == -1)
    {
